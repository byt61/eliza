...
describe("Signal message connector", () => {
  ...
  it("sorts connector targets safely when scores contain NaN", () => {
    const targets = [
      { name: "Target B", score: NaN, target: { channelId: "b" } },
      { name: "Target A", score: 0.9, target: { channelId: "a" } },
    ];
    ...
    targets.sort((left, right) => {
      ...
      return (
        ...
        (left.name ?? left.target.channelId ?? "").localeCompare(
          right.name ?? right.target.channelId ?? ""
        )
      );
    });

    expect(targets[0]?.name).toBe("Target A");
    expect(targets[1]?.name).toBe("Target B");
  });
});
